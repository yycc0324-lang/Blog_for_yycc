#!/usr/bin/env bash
# ============================================================
# Shirone 博客 · Meting 服务「登录 Cookie」更新脚本
# ------------------------------------------------------------
...
# ----------------------------- 配置区 -----------------------------
# 部署目录：**留空即自动识别** —— 先看 ~/shirone-meting 与 /www/wwwroot/shirone-meting，
# 都没有时再读「正在运行的 Meting 容器」的挂载反推（因此与其它站点共用容器也能认出来）。
# 需要强制指定时：DEPLOY_DIR=/你的目录 bash scripts/update-meting-cookie.sh
DEPLOY_DIR="${DEPLOY_DIR:-}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=scripts/meting-common.sh
source "$SCRIPT_DIR/meting-common.sh"
# ------------------------------------------------------------------
...
COOKIE_FILE=""   # 待解析出 DEPLOY_DIR 后再赋值（见下方 mt_resolve_deploy_dir）
SRC_FILE=""
...
# ---------- 1.1) 解析部署目录（自动识别，无需 DEPLOY_DIR=... 前缀）----------
