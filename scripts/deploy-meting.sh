#!/usr/bin/env bash
# ============================================================
# Shirone 博客 · 自建 Meting 音乐接口一键部署脚本（Docker 版）
# ------------------------------------------------------------
# 用法（在服务器上执行）：
...
PROJECT_DIR="${PROJECT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
                                    # 博客源码目录（默认脚本的上级目录）
DEPLOY_DIR="${DEPLOY_DIR:-}"        # 留空 = 自动识别（见 scripts/meting-common.sh）；
                                    # 显式传入则优先使用，不再自动识别
                                    # 音乐服务部署目录（与博客站点目录分开）
# 公共库：负责「部署目录自动识别」，日常维护无需再写 DEPLOY_DIR=... 前缀
# （仓库内为单一真源，update-meting-cookie.sh 复用同一份逻辑）
# shellcheck source=scripts/meting-common.sh
source "$PROJECT_DIR/scripts/meting-common.sh"
...
METING_PORT="${METING_PORT:-}"      # 留空 = 自动：优先用「运行中容器」的端口，否则 8900
METING_PORT_EXPLICIT="$METING_PORT"
                                    # 宿主机回环端口（只绑 127.0.0.1，不对外）
...
#   服务器：/www/wwwroot/shirone-meting —— root/www 用户可直接创建
#   本地开发机：/www 通常只读，创建失败则回退 ~/shirone-meting
#   已存在容器（含与其它站点共用）时：直接读容器挂载反推，无需任何参数
# 具体逻辑见 scripts/meting-common.sh 的 mt_resolve_deploy_dir()，
# 在下方解析完 ACTION 后调用（例如 test/logs/down 不必依赖「配置区」默认值）。

ACTION="${1:-up}"

# 解析部署目录：带 DEPLOY_DIR=... 时用传入值，否则自动识别
mt_resolve_deploy_dir

# 端口解析：显式指定 > 运行中容器的端口 > 仓库默认 8900
METING_PORT="${METING_PORT_EXPLICIT:-${MT_DETECTED_PORT:-8900}}"
...
prepare_dir() {
	...
	# 端口与 METING_PORT 保持一致（仅在显式指定 METING_PORT 时改写，
	# 避免把仓库默认的 8900 改成「其它站点正在用的端口」）
	if [ -n "$METING_PORT_EXPLICIT" ] && [ -f "$DEPLOY_DIR/docker-compose.yml" ]; then
		sed_inplace "s|127.0.0.1:[0-9]*:80|127.0.0.1:${METING_PORT}:80|" "$DEPLOY_DIR/docker-compose.yml"
		log "已把 compose 端口同步为 ${METING_PORT}"
	fi
...
patch_index_php() {
	...
	if grep -q "shirone-meting:cookie" "$file"; then
		info "index.php 已包含 Cookie 读取逻辑，跳过注入"
	else
		cat > /tmp/shirone-meting-cookie.php <<'COOKIEEOF'
/* shirone-meting:cookie —— 由 scripts/deploy-meting.sh 生成：运行时读取挂载的 qq-cookie.txt */
...
COOKIEEOF
		sed_inplace "/^\$api->format(true);/r /tmp/shirone-meting-cookie.php" "$file"
		rm -f /tmp/shirone-meting-cookie.php
		grep -q "shirone-meting:cookie" "$file" || die "Cookie 注入失败，请手动修改 $file"
...
────────────────────────────────────────────────────────────
✓ 博客前端只需改 src/config/musicConfig.ts 的 meting 段：

   provider: "mixed",
   meting: {
     api: "${PUBLIC_SCHEME}://${PUBLIC_DOMAIN}/?server=:server&type=:type&id=:id&r=:r",
     server: "${TEST_SERVER}",
     type: "playlist",
     id: "${TEST_PLAYLIST_ID}",
   },

改完后重新构建发布：pnpm build（再把 dist 部署到博客站点）
（**换歌单 = 只改上面的 id 再重新构建发布**；换平台要同时改 server，
  例如腾讯的歌单 id 配 netease 会返回空歌单）
...
case "$ACTION" in
	up)
		# 已在运行「共用容器」时，本命令默认另起一套（仓库默认 8900），避免端口冲突
		if [ -z "$METING_PORT_EXPLICIT" ] && [ -n "${MT_DETECTED_PORT:-}" ]; then
			METING_PORT=8900
			warn "检测到共用容器在 127.0.0.1:${MT_DETECTED_PORT}，本命令将单独部署一套（端口 ${METING_PORT}）"
			warn "  只想共用现成容器：不必执行本命令，直接 bash scripts/deploy-meting.sh test 自测即可"
		fi
		ensure_docker
		prepare_dir
		patch_index_php
	...
	refresh)
		...
		if [ -d "$DEPLOY_DIR/meting-api/cache/playlist" ]; then
			...
			warn "  当前部署目录：$DEPLOY_DIR（需要指定时：DEPLOY_DIR=/你的目录 bash scripts/deploy-meting.sh refresh）"
		fi
		run_selftest
		;;
	restart)
