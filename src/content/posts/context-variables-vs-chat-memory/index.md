...
对象创建完之后就一直放在那里，等 Swarm 来读。Agent 本身不承担任何计算，作用只是把配置存住。

至于"它全程没传递东西"，这个观察是对的，而且这正是设计意图。Agent 是无状态的配置对象，可以预先创建、反复复用。如果让 Agent 自己往外推数据，它就得维护状态，还得知道当前在跟谁说话、消息发到哪了，那这部分工作就和 Swarm 重叠了。

:::tip 这样设计的好处
因为 Agent 只是一份配置，同一个 Agent 对象可以塞给多次 `client.run()` 复用。需要不同行为时，另外创建一个 Agent 就行，不用去改框架本身。
:::
...
我感觉还说先把结论给你们，下面的定义有点太多了
如果是LangChain的多轮对话记忆
你只是想让 Chatbot 记住刚才聊了什么，实现多轮连续对话。
你的需求是标准的“记住聊天记录”，不需要在对话历史之外维护额外的业务变量。
作为快速原型，先用一行代码验证对话记忆的效果。
-------------------------
如果需要操作“上下文变量”（自定义 State）的场景：
多步骤任务：Agent 需要在运行中记录中间结果（如先搜资料 A，再结合资料 B 写报告），这些中间变量必须存在 State 中，供后续步骤读取
动态行为：你需要根据用户是否登录、对话轮数等变量，动态调整系统提示词或可用工具。
工具交互：工具函数需要读取或写入会话级别的特定变量（如当前任务 ID），而不仅仅是收发消息。
-----------------------
...
它就是个普通 Python 字典，`client.run()` 开始时创建，运行一结束就没了。框架不负责持久化。
...
LangChain 的 Memory 关心的是数据存在哪里、怎么持久化。Swarm 的 Context Variables 关心的是同一次运行里，不同组件怎么拿到同一份数据。

![Swarm 单次运行内的完整交互流程](./swarm-tool-call-flow.webp)

*图：单次 `client.run()` 里，用户代码、Swarm 框架、AI 模型、功能代码之间如何传递消息和 `context_variables`。*

平时大家爱把这两件事概括成"一个管传，一个管存"。这么说确实好记，但容易让人以为 `context_variables` 不存东西。它其实是存的，`context_variables = {"user_id": 456}` 就是把 456 存了起来。

所以要真正分清楚，还是得看这份数据能活多久、谁能看见它，以及运行结束后由谁负责把它留下来。
