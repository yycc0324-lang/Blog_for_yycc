...
注意箭头方向：**是 Swarm 主动去「获取」Agent 的属性，不是 Agent 主动「推」给 Swarm。**
...
对象创建完就静静待着。它不执行、不推理、也不调用任何函数，就是一张**等着被查询的配置表**。

至于「它全程没传递东西」——这个观察是对的，而且这正是设计意图。Agent 是**无状态的配置对象**，可以预先创建、反复复用；如果它主动往外推数据，就得自己维护状态、知道当前在跟谁说话、消息发到哪了，那它就不是 Agent，而是 Swarm 了。
...
说白了就是：**指令函数是「你写、Swarm 调、结果变成提示词」；Tool 是「你写、AI 决定调、结果变成工具返回」。**

也正因为这样，指令函数不做任何业务操作——它不查数据库、不调 API，只负责根据上下文现场拼一段提示词出来。真正干活的是 Tool。

### 2.1 那「很多段动态 prompt」是怎么选的

可能你会想：一个 Agent 可以准备很多段动态 prompt，Swarm 怎么确认这次用哪一段？靠 AI 判断吗？

不靠 AI，而且这里根本没有「从多个 prompt 里挑一个」这个动作。
...
所谓的「很多段动态 prompt」，其实是**同一个函数，根据不同的 `context_variables` 返回不同的字符串**：
...
选择逻辑是你自己写的 `if-else`，判断依据是 `context_variables`，执行者是 Python 解释器。**整个过程里 AI 模型还没出场**——Swarm 是在「准备调用 AI」的阶段就把这段提示词定好了，然后才连同对话历史、工具 schema 一起发给模型。AI 看到的只是结果。

### 2.2 顺带一个细节：Tool 怎么拿到上下文

AI 决定调用工具时，**只会提取用户明确说出来的参数**（比如 `location="Paris"`）。它不会、也不该去管 `context_variables`——那是框架内部的共享状态，不是从对话里能提取出来的东西。
...
翻译一下：**你这个工具函数的参数列表里如果写了 `context_variables`，Swarm 就自动把当前上下文塞进去；没写就不塞。**

这样你的工具函数就能同时拿到两样东西：AI 提取的业务参数，加上 Swarm 维护的上下文。

## 3. 模型不是 Swarm 挑的

可能你会想：Swarm 拿到提示词和工具之后，会判断该用哪个模型吧？

不会。模型名是 **Agent 自己配好的**，Swarm 只是读它，不是选它。
...
它不会说「这任务简单，换个便宜的」，也不会说「这任务复杂，升到 GPT-4」。Agent 说用什么，Swarm 就用什么。

模型唯一会变的情况是 **handoff**：当前 Agent 把对话交给另一个 Agent，Swarm 更新 `active_agent`，下一轮循环重新去读新 Agent 的 `agent.model`。变的原因是换了 Agent，不是 Swarm 自己判断出来的。
...
真正需要算力的「判断」其实只有一件事：**用户这句话，该不该触发某个函数调用。** 该调退款函数还是天气函数，还是什么都不调直接回复——这才是模型算出来的。
...
所以更准确的说法是：**Agent 提供「我是谁、我能干什么、我能找谁」的声明，AI 模型在这些声明范围内做「现在该干什么」的判断，Swarm 把判断落地执行。**

顺便纠正一个容易想歪的地方：**Agent 对象里并不「包含」AI 模型**，它只是存了模型的名字（`"gpt-4o"`）。真正的模型在别人的服务器上。所以更准确的写法是：
...
先说第一问：**智能体对象才是 Agent**，AI 模型只是它借用的「大脑」。

第二问没有单一答案，因为这件事被拆成了三段，各管一段：

![handoff 的三段分工](./handoff-division.webp)

*图：「调不调」「去哪」「怎么转」分别由三个角色负责。*
...
这点很容易被说糊涂。我第一次讲的时候就直接说了句「是 AI 决定找哪个专家」，后来发现不对。准确的说法应该是：

> **AI 模型决定「要不要触发转交」，程序员决定「转交给谁」。**

这两件事是分开的。如果程序员不写那个函数，AI 模型永远无法转交——它的工具列表里根本没有这个选项。

## 5. 小结

到这儿第一部分可以收一下了：

- **Agent 对象**是定义者：我是谁、我怎么说话、我能用什么工具、我能转交给谁、我用哪个大脑；
- **AI 模型**是决策者：在当前 Agent 划定的范围内，用算力决定现在该做什么；
- **Swarm 框架**是执行者：读配置、组请求、发模型、执行工具、切换 Agent。

一句话：**Agent 是身份，AI 模型是大脑，Swarm 是手脚。**

# 第二部分　context_variables 到底是个什么东西

角色分清了，再回头看这个上下文变量，有三个容易被忽略的细节。
...
- **你传进去的**是初始上下文，这个原始字典全程不会被改动；
- **运行时用的**是深拷贝出来的副本；
- **你拿回来的**是运行过程中被工具函数更新过的副本。

想持久化这次运行产生的新数据，得自己从 `response.context_variables` 里取出来存。你原来那个字典从头到尾是干净的。
...
原因很简单，`context_variables` 只活在单次运行的内存里。刷新网页本质上就是：原来的进程/会话结束，内存里的字典被回收；新会话调用 `client.run()` 时你不主动传，Swarm 拿到的就是空字典。

要想「还记得」，你得自己做三件事——存、取、传：
...
这里还有个容易混的点：`messages` 和 `context_variables` 是两回事。如果你把 `messages` 恢复了，AI 能从文本里「看到」之前聊过什么——但那是 AI 在读文本，不是 `context_variables` 自动恢复了。

# 第三部分　那两种「记忆」的区别

到这儿，终于可以正面回答最开始那个问题了。

## 1. 一个管传，一个管存

- **LangChain 的 Memory**，是「存东西的地方」——重点在数据放在哪、怎么持久化；
- **Swarm 的 Context Variables**，是「传东西的管子」——重点在同一次运行里，不同组件怎么拿到同一份数据。

不过得诚实补一句，免得这个说法把人绕进去：**把数据放进字典，本身也是一种「存」。** `context_variables = {"user_id": 456}` 当然是把 456 存起来了。

所以「传 vs 存」只是为了好记的简化说法。真要分清楚，得看三件事：**生命周期**（活多久）、**作用域**（谁能看见）、**持久化归谁管**（谁负责在运行结束后把它留下来）。
...
*图：`context_variables` 只在单次 run 内活着；ChatMemory 配了后端才能跨轮存活。*
...
- 你要的是**对话历史的持久化**（刷新后还能看到之前聊过什么），那 LangChain 的 Memory 加持久化后端确实更省事；
- 你要的是**多 Agent 之间共享结构化状态**（`user_id`、账户类型这种字段，不想塞进对话文本里），那 `context_variables` 是原生设计，LangChain 反而没有直接对应的机制，得自己搭。
...
*图：对话全文交给模型理解；只有需要显式共享的已知字段才进字典。*

- `messages` 负责「理解」：把完整对话交给模型，它自己就能读出「用户叫小明」「在 A 高中」「赶时间」；
- `context_variables` 负责「共享」：只有当某个信息**需要被跨 Agent、跨函数显式传递**时，才把它放进字典。

这里最关键的一点是：**「提取」这个动作是开发者做的，Swarm 不会自动干。**

它不会去原文里检索关键词，也不会判断「小美」算不算关键变量。Swarm 只负责三件事：把字典传进 `run()`、handoff 时带着它走、把工具函数写回的值合并进去。

字段来源通常有三种：你自己写规则提取、你调一次 LLM 做结构化抽取、你写的工具函数通过 `Result(context_variables={...})` 写回。

## 5. 两边的边界都在哪

**Context Variables 不太能指望的：**

- **不自动提取**——它是个空箱子，放什么完全由你和你的函数决定；
- **只适合预设场景**——你得提前知道有哪些字段要共享（客服、订票、账户查询这类流程固定的业务）；
- **不适合理解开放文本**——用户输入完全开放时，提前定义字段是徒劳的，这种场景交给 `messages`；
- **不持久化**，运行结束就没了；
- **容量小**——官方定位是 KB 级，塞大段文本只会拖慢执行，因为它会被传给 instructions 函数和所有工具函数。

**Memory / ChatMemory 不太能指望的：**

- **持久化是配出来的**，不是自带的；
- **不负责跨 Agent 共享**——LangChain 的多 Agent 通常走工具调用/任务分发，各子 Agent 有自己的上下文，要共享得自己在应用层搭桥；
...
2. **`context_variables` 管「传」。** 单次运行内的共享状态，随 handoff 自动流动，运行完就没了，而且它不会自动提取字段——要持久化得自己「存、取、传」。
3. **LangChain 的 Memory 管「存」。** 它默认也是内存、刷新也会丢；它比 Swarm 多的不是「持久化能力」，而是**现成的持久化插件**。开放文本的理解交给 `messages`，只有「已知、且需要跨组件显式共享」的字段，才放进 `context_variables`。
