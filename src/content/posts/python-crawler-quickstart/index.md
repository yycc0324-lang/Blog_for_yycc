---
title: "Python爬虫入门推荐：简单的脚本可以快速建立学习兴趣"
published: 2026-09-17
description: "写给刚学完 Python基础语法、但是不知道这门语言有什么用，想立刻看到成果的新生，爬些感兴趣的东西可能是让你进入脚本世界的最好选择：从浏览器 DevTools 找线索，用 requests + lxml 一步步抓下整本小说。"
...
不需要框架，如果你不看什么乱七八糟的教程（比如余胜军），也不用一上来就啃各种底层。你只要会两件事：**懵懂的发一个 HTTP 请求**，**感受一下，从返回的 HTML 里把想要的文字挑出来**。
...
网站用的是 **GET**，所以代码里也用 `.get()`。如果人家用的是 POST，你就得改成 `.post()` 并按表单格式传参数，方法要跟网站一致，不是你想用哪个就用哪个，至于什么是GET，什么事POST，相信你可以从各种AI处得到答案。

## 3. 伪装成浏览器：User-Agent

### 3.1 为什么会被拒绝

你在浏览器里读小说，网站把你当"正常访客"；但代码发出去的请求带着一身"我是程序"的痕迹，那你的胆子真是肥嘟嘟的，这很容易被识别出来，然后被拒之门外，如果你之前了解过某些网站短时间被大量同一IP访问会被封禁的概率很高这一概念的话但不知道为什么的话，结合上述内容把不懂的内容再问问DeepSeek大人，你现在应该知道为什么了。
...
（UTF-8是什么呢，你知道的朋友，请移步Deep Seek或者各类计算机科普视频）好多了。但现在拿到的还是**整页 HTML**，导航、广告、"推荐阅读"全混在里面。我们要的只是正文。
